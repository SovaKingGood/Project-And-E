...
void uni_hid_parser_ds4_setup(struct uni_hid_device_s* d) {
    ...
    if (!child) {
        logi("DS4: Virtual device creation skipped (virtual devices disabled). Gamepad will work without mouse support.\n");
        // Parent device is already ready, safe to continue without virtual device
        return;
    }
